...
class ConversionUtils {
  companion object {
    ...
    @Throws(JSONException::class)
    fun convertArrayToByteArray(readableArray: ReadableArray?): ByteArray? {
      val array = ByteArray(32)
      for (i in 0 until readableArray!!.size()) {
        if (readableArray.getType(i) === ReadableType.Number) {
          array[i] = readableArray.getDouble(i).toByte()
        }
      }
      return array
    }
    ...
    @Throws(JSONException::class)
    fun convertByteArrayToArray(byteArray: ByteArray): WritableArray {
      val array = WritableNativeArray()

      for (i in byteArray.indices) {
        array.pushInt(byteArray[i].toInt())
      }
